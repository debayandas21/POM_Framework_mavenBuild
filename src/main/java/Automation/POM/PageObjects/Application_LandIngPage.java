...
public class Application_LandIngPage extends BaseClass {
	 ...
	 * @throws Exception
	 */
	public void perform_signIn(ExtentTest report) throws Exception
	{
		try {
			
			actions.sendText(UserNameField, globalConfig.globalParameter.get("UserName"));
			actions.sendText(passwordFld, decodeEncryptedData(globalConfig.globalParameter.get("Password")));
